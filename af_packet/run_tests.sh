...
#
# BANDWIDTH TEST (iperf3 UDP):
#   results/<auth>/<iface>/iperf3/<ts>/results.csv
#   results/<auth>/<iface>/iperf3/<ts>/run_<n>/sz_<size>/iperf3_raw.txt
#
# RTT TEST (sockperf ping-pong --full-rtt):
#   results/<auth>/<iface>/rtt/<ts>/results.csv
#   results/<auth>/<iface>/rtt/<ts>/run_<n>/sz_<size>/rtt_raw.txt
#
# Override logic (NEW):
# - Ask default runs for ALL sizes.
# - Ask override runs for sizes {32, 1024, 8192}. If override>0, run those sizes override times.
...
run_sockperf_pingpong_once() {
  # prints: min,avg,p50,p99,max,timeout
...
run_sender_mode() {
  ...
  for sz in "${SIZES[@]}"; do
    ...
    echo "=== Size ${sz} bytes: ${runs_for_size} runs (each ${DURATION_SEC}s) ==="
...
main() {
  ...
  if [[ "$role" == "sender" ]]; then
    if [[ "$TEST_KIND" == "bandwidth" ]]; then
      runs="$(ask_int "How many bandwidth runs per msg_size (default for all sizes)?" "32")"
  ...
  if [[ "$TEST_KIND" == "bandwidth" ]]; then
    if [[ "$role" == "server" ]]; then
      ...
      run_sender_mode "$label" "$host_ip" "$peer_ip" "$stamp" "$runs" "$runs_override"
    fi
    exit 0
  fi
